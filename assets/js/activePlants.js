        ...
        if (document.getElementById(deviceName)) {
          ...
          for (var m = 0; m < Firestoredata.length; m++) {
            if (Firestoredata[m].DeviceID === deviceName) {
              ...
              div.innerHTML = "<div style='height: 100%; border-radius: 25px' class='p-2 bg-primary bg-gradient text-white'><h3 class='text-center'>" + Firestoredata[m].DeviceName + "</h3>" + '<p class="m-0">Taluqa: ' + Firestoredata[m].Taluqa + '</p><p class="m-0">Location: ' + Firestoredata[m].Location + '</p></div>';
            }
          }
          // div = document.createElement('div');
          // div.id = deviceName;
